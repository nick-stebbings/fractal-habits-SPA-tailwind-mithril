  ...
  module Repos
    class DateRepo < ROM::Repository[:dates]
      ...
      def delete(primary_key)
        Hht::Transactions::Dates::Delete.new.call(primary_key)
      end
      ...
      def find(given_date)
        dates.where(h_date: given_date)
      end

      def earliest
        dates.order(:h_date).first
      end

      def latest
        dates.order(:h_date).last
      end

      def all_after(given_date)
        dates.where { h_date >= given_date }
      end
      ...
      def all_as_json
        { dates: dates.order(:h_date).map do |date|
          {
            'id' => date.fetch(:id),
            'h_date' => date.fetch(:h_date) # habit_date
          }
