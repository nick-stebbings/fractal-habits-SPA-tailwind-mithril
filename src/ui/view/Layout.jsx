...
const defaultHabit = () =>
  [
    ...
    "There are no habits yet for this domain",
  ].includes(HabitStore.current()?.name);
...
export default {
  ...
  oninit: ({ attrs: { spinnerState } }) => {
    ...
    if (m.route.param("demo")) {
      return console.log("Demo state recognised");
    }
    if (
      (!defaultHabit() &&
        !fetching() &&
      ...
      let treeReload = isVisPage() && loadTreeData().then(m.redraw);
      changeOfModelContext() ||
        (changedHabit() &&
          fetching(true) &&
          spinnerState(true) &&
          openModal(true) &&
          Promise.all([treeReload, preLoadHabitDateData()])
            .then(() => populateCalendar())
            .then(() => clearTheDecks(spinnerState)));

      Promise.all([dateReload, habitReload])
        .then(() => preLoadHabitDateData())
        .then(
          () =>
            (newRecord() || newDate() || changedHabit()) && populateCalendar()
        )
        .then(() => {
  ...
  view: ({
    ...
    <div>
      ...
      <LogoLink handleClick={changedHabit} />
      <div
        id="app"
